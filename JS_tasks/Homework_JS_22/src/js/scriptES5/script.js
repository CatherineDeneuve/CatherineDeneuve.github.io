...
var test = {
  ...
  createHeader: function createHeader(textHeader) {
  ...
  createOl: function createOl() {
    ol = document.createElement('ol');
    this.form.appendChild(ol);
  },

  getGrid: function getGrid(myVar) {
  ...
  createLi: function createLi(textLi) {
  ...
  createInput: function createInput(textLabel) {
  ...
  changeTypeForInput: function changeTypeForInput() {
  ...
  for (var i = 0; i < myObj.length; i++) {
    ...
    for (var j = 0; j < myObj[i].answers.length; j++) {
      ...
      if (inputs[a].checked == myObj[i].answers[j].correct) {
        ...
        if (myObj[i].answers[j].correct === false) textResult += myObj[i].answers[j].text + "<br/>";
      }
      a++;
    }
