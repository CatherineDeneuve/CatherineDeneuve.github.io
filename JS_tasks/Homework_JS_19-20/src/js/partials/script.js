$(function() {
    ...
    var pixelsOffset = 1145;
    var currentLeftValue = 0;

    input1.click(function() {
      currentLeftValue = 0;
        elementsList.animate({ left : currentLeftValue + "px"}, 'fast');
    });

    input2.click(function() {
      currentLeftValue = -1145;
        elementsList.animate({ left : currentLeftValue + "px"}, 'fast');
    });

    input3.click(function() {
      currentLeftValue = -2290;
        elementsList.animate({ left : currentLeftValue + "px"}, 'fast');
    });
...
var skills = _.sortBy(_.union(_.flattenDeep(_.map(obj,'skills'))));
console.log ("skills", skills);

// 2. Массив имен (поле name) людей, отсортированных в зависимости от количества их друзей (friends)

var friendsArray = _.map(obj, 'friends');
var namesArray0 = _.map(obj, 'name');

var zip = _.zip(friendsArray, namesArray0);
var zip = _.zip(_.map(obj, 'friends'), _.map(obj, 'name'));
var zipMap = _.map(zip, function(value){
  return _.flatten(value);
});
var sortBy2 = _.sortBy(zipMap);
var flatten = _.flattenDepth(sortBy2, 2);
var mapLast = _.map(flatten, function(value){
  if( _.isString(value)){
    return value;
  }
});

var compact = _.compact(mapLast);
console.log('compact', compact);

// 3. Массив всех друзей всех пользователей, не должно быть повторяющихся людей

var flattenFriends = _.flattenDeep(friendsArray);
var friendsFullList = _.map(flattenFriends, 'name');
var unionFriends = _.union(friendsFullList);
console.log('unionFriends', unionFriends);

// console.log('zip', zip);
// console.log('zipMap', zipMap);
// console.log('sortBy2', sortBy2);
// console.log('flatten', flatten);
// console.log('mapLast', mapLast);
